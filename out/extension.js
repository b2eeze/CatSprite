...
exports.deactivate = exports.activate = exports.ShowInputBox_secret = exports.ShowInputBox_api = void 0;
...
let API_KEY = vscode_1.workspace.getConfiguration().get('catsprite.apiKey');
let SECRET_KEY = vscode_1.workspace.getConfiguration().get('catsprite.secretKey');
...
async function fetchAccessToken() {
    const accessTokenRes = await axios_1.default.post(ACCESS_TOKEN_URL, null, {
        params: {
...
async function aiProcess(question) {
    const messages = [
    ...
    const res = await axios_1.default.post(CHAT_URL, { messages }, { params: { 'access_token': token } });
    let data = '';
    if (res) {
...
async function ShowInputBox_api() {
    ...
    await vscode_1.workspace.getConfiguration().update("catsprite.apiKey", result, true);
    // Write to workspace settings
    return result;
}
exports.ShowInputBox_api = ShowInputBox_api;
// show an input box and get api key
async function ShowInputBox_secret() {
    const result = await vscode_1.window.showInputBox({
        ignoreFocusOut: true,
    ...
    vscode_1.window.showInformationMessage(`Got: ${result}`);
    // Write to user settings
    await vscode_1.workspace.getConfiguration().update("catsprite.secretKey", result, true);
    // Write to workspace settings
    return result;
}
exports.ShowInputBox_secret = ShowInputBox_secret;
...
async function activate(context) {
    // ============================== enable condelens ================================== //
    const funccodelensProvider = new CodelensProvider_1.FuncCodelensProvider();
    vscode_1.languages.registerCodeLensProvider("*", funccodelensProvider);
    const ImpcodelensProvider = new CodelensProvider_1.ImpCodelensProvider();
    vscode_1.languages.registerCodeLensProvider("*", ImpcodelensProvider);
    // ============================== initialization =================================== //
    // get API key from the user
    if (vscode_1.workspace.getConfiguration().get("catsprite.apiKey") === "") {
        API_KEY = await ShowInputBox_api();
        console.log(API_KEY);
    }
    if (vscode_1.workspace.getConfiguration().get("catsprite.secretKey") === "") {
    ...
    context.subscriptions.push(vscode_1.commands.registerCommand('catsprite.helloWorld', () => {
        // executed every time your command is executed
        // Display a message box to the user
        vscode_1.window.showInformationMessage('Meow meow! 🐾🐾Hello, I\'m your adorable cat sprite🐈! ');
        vscode_1.window.showInformationMessage('Even though I\'m not a real cat, I can still keep you company, play with you, and explore the world together!😽 Meow~');
    }));
    vscode_1.commands.executeCommand('catsprite.helloWorld');
    context.subscriptions.push(vscode_1.commands.registerCommand("catsprite.start", () => {
        currentPanel = vscode_1.window.createWebviewPanel('catCoding', 'Cat Coding', vscode_1.ViewColumn.One, {
        ...
        const updateWebview = () => {
            ...
            currentPanel.webview.onDidReceiveMessage(message => {
                if (message.method === 'showMessage') {
                    vscode_1.window.showInformationMessage(message.params.content);
                }
    ...
    vscode_1.commands.registerCommand("catsprite.enableCodeLens", () => {
        vscode_1.workspace.getConfiguration("catsprite").update("enableCodeLens", true, true);
    });
    vscode_1.commands.registerCommand("catsprite.disableCodeLens", () => {
        vscode_1.workspace.getConfiguration("catsprite").update("enableCodeLens", false, true);
    });
    vscode_1.commands.registerCommand("catsprite.codelensAction", async (arg) => {
        // ask baidu api for advice
        vscode_1.window.showInformationMessage(`😸Cat Sprite is thinking, please waiting...........`);
        const result = await aiProcess(arg);
        vscode_1.window.showInformationMessage(`😺Cat Sprite: ${result}$`);
    });
}
